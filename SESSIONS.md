...
| 2026-03-18 | PostPhotoSheet + photo card polish — redesigned `PostPhotoSheet` to match voice sheet's crafted feel: wrapped photo area in same `rounded-xl border bg-[#F8F4EE]` container, added 68px camera button with `btn-primary` + shadow (matching mic button pattern), wired real drag-and-drop (`onDragOver`/`onDragLeave`/`onDrop` with green border/bg feedback), replaced upload spinner pill with bottom-anchored gradient overlay + indeterminate progress bar, streamlined file info row (filename pill + ghost "Change" button, removed redundant "Remove" text button). Photo card upgraded: shimmer replaced with blurred gradient placeholder, Instagram-style `aspect-[4/5]` + `object-cover` framing, subtle hover lift (`scale(1.005)` + shadow). Same aspect ratio applied to sheet preview. | **`object-contain` dead space** — initial attempt used `object-contain` to respect aspect ratio, but created large empty areas around small/non-standard images. Fix: switched to Instagram's approach — fixed `aspect-[4/5]` container with `object-cover` for edge-to-edge fill with center crop. |
