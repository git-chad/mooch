...
### Drag offset caused by motion transform wrappers on kanban columns
- **Problem:** `TransitionSlot` wraps children in `motion.div` with `y: 10` + `filter: blur(6px)` entrance animation. `@hello-pangea/dnd` calculates drag positions from DOM offsets, which are wrong when parent elements have active CSS transforms. Cards appeared far from the cursor when dragged.
- **Fix:** Removed `TransitionSlot` wrapper from `KanbanBoard` entirely for the column grid. Also removed stagger `motion.div` wrappers around individual columns.
- **Avoid:** Never wrap `@hello-pangea/dnd` droppable/draggable elements in motion containers that apply CSS transforms (`translateY`, `scale`, `rotate`, `filter`). If entrance animation is needed, animate properties that don't affect layout positioning (e.g. `opacity` only).

---

### Flex gap causes vertical jump on card drop in dnd columns
- **Problem:** `gap-3` on the droppable flex column caused cards to jump vertically when dropped. `@hello-pangea/dnd`'s placeholder element only accounts for element height, not CSS `gap` spacing between flex children.
- **Fix:** Removed `gap-3` from the droppable container. Added `mb-3 last:mb-0` on each draggable child instead, so spacing is part of the element's own box model.
- **Avoid:** For `@hello-pangea/dnd` droppable containers, use margin on children instead of flex `gap` for spacing. The library's placeholder doesn't account for gap.

---

### Dialog/Sheet backdrop renders under nav elements
- **Problem:** Sheet, ConfirmDialog, and Modal backdrops and viewports had no `z-index` set (defaulting to `z-auto`). Navigation elements (MobileTopBar `z-40`, ShellTopNav `z-30`, BottomTabBar `z-40`) stacked above them, causing the dark overlay and dialog content to appear behind the nav.
- **Fix:** Added `z-50` to both the `Dialog.Backdrop` and `Dialog.Viewport` in all three components (Sheet, ConfirmDialog, Modal).
- **Avoid:** All portal-based overlays (modals, sheets, dialogs, popovers) must have a z-index higher than the highest navigation element. Currently nav uses `z-30`/`z-40`, so overlays should use `z-50`.

---

### "Create event" badge overlapped card metadata
- **Problem:** The "Create event" hover badge was absolutely positioned (`absolute bottom-3 right-3`) over the card content area. When a card had a date badge in the footer, the "Create event" badge covered it.
- **Fix:** Moved the badge from absolute positioning into the card's inline footer row with `pointer-events-auto` (since the footer is inside a `pointer-events-none` content wrapper). It now flows naturally with other footer elements.
- **Avoid:** Avoid absolute-positioning interactive elements over card content that may have variable metadata. Prefer inline placement in footer/action rows.
...
| 2026-03-22 | Phase 5 Kanban UI polish pass — Fixed Codex-generated low-quality UI across the kanban board. **KanbanBoard:** removed `TransitionSlot` and stagger `motion.div` wrappers (caused drag offset), uses shared `EmptyState`, `AnimatePresence` for empty→board transition. **KanbanColumn:** per-column color accents (amber/brown/green/gray for Ideas/To Plan/Upcoming/Completed), card count in header, empty column placeholder text, column-tinted drop zone on drag-over, `mb-3` margin instead of flex `gap` (fixes dnd placeholder jump). **PlanCard:** `rounded-[14px]` + Badge shadow language, footer with creator avatar + `relativeTime()` + date badge + organizer avatar, "Create event" moved from absolute overlay to inline footer (for upcoming cards only), removed photo/voice attachment display. **CreatePlanSheet:** removed photo/voice upload, replaced native `<input type="datetime-local">` with shared `DateTimePicker` (matches polls), `Select` dropdown for status, `Input` for title. **PlanDetailPanel:** status `Select` with optimistic updates via Zustand store, `Button variant="inline"` / `"inline-danger"` for Edit/Delete actions. **Button component:** added `inline` and `inline-danger` variants for lightweight text+icon action buttons. **plan-status config:** added `color` hex and `emptyLabel` per status, renamed "Done" → "Completed". **Dialog z-index fix:** added `z-50` to backdrop+viewport in Sheet, ConfirmDialog, and Modal (nav was `z-30`/`z-40`, overlays had no z-index). | **Drag offset from motion transforms** — `TransitionSlot` applies `y:10` + `blur(6px)` that `@hello-pangea/dnd` can't compensate for. Fix: removed `TransitionSlot` wrapper. **Flex gap dnd jump** — `gap-3` on droppable flex column caused vertical jump on drop (placeholder doesn't account for gap). Fix: `mb-3 last:mb-0` on children. **Ghost variant buttons turned green on hover** — Edit used `variant="ghost"` (lime accent), Delete override got clobbered. Fix: created dedicated `inline`/`inline-danger` Button variants. **Stale `.next` cache** — buttons looked different between branches despite identical CSS. Fix: delete `.next/` folder. **Dialog backdrop under nav** — Sheet/ConfirmDialog/Modal backdrops had no z-index, nav had `z-30`/`z-40`. Fix: `z-50` on backdrop+viewport. **"Create event" overlapped date badge** — absolute positioning covered footer metadata. Fix: moved inline into footer row. |
