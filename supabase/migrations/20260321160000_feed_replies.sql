...
begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'feed_replies' and policyname = 'Group members can read replies'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'feed_replies' and policyname = 'Group members can insert replies'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'feed_replies' and policyname = 'Users can delete own replies'
  ) then
...
create index if not exists feed_replies_feed_item_id_created_at_idx
  on public.feed_replies (feed_item_id, created_at asc);
